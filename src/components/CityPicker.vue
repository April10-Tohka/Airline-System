...
import {getSearchBoxRecommend} from "@/api/restapi.js"
onBeforeMount(()=>{
    getSearchBoxRecommend()
        .then(res=>{
            recommendCity.value=res.data.recommendCity.cityList;
            cityPickerTabBar.value=res.data.cityPickerTabBar;
        ...
        <div class="city-picker-selector" v-show="panelState" @mousedown="handleMouseDown">
            ...
            <div class="city-picker-selector-content">
                <ul class="city-picker-tabs">
                    <li class="city-picker-tab-item"  :class="{active:tab===activeTab}"
                    v-for="(tab,index) in cityPickerTabBar" :key="index" @click="selectTab(tab)">
                        {{tab}}
                    </li>
                </ul>
                <div class="city-picker-body">
                    <ul class="hot-cities" v-if="activeTab==='热门'">
                        <li v-for="city in recommendCity" :key="city.id" @click="selectCity(city)">
                            {{city.name}}
                        </li>
                    </ul>
                    <div class="domestic-city-item" v-else
                    v-for="(cities,key) in filterCities" :key="key" >
